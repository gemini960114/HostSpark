...
- Windows 開發與模擬測試：已完成。
- Ubuntu、AGY CLI、Telegram Bot API、systemd 端到端測試：尚未執行。
- Git commit／push：尚未執行。
- 正式 VM 部署：尚未執行。
...
- [ ] Ubuntu 依賴安裝成功，`pip check` 通過。
- [ ] AGY headless 執行成功且支援 `--add-dir`。
- [ ] 完整測試與 `--check-config` 通過。
- [ ] systemd service 穩定運行。
- [ ] 舊有 `/status`、一般問答、`/clear` 沒有回歸。
- [ ] 未授權 Telegram 帳號無法操作。
- [ ] 排程可新增、預覽、取消與確認。
- [ ] 排程可列出、查看、暫停、恢復與刪除。
- [ ] Bot 可在沒有人工提問時主動回報。
